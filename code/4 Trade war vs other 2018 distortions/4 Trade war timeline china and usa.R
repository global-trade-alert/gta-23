...
# gta_data_slicer(intervention.ids = )
...
# 
# trade.china.to.usa <- subset(trade, i.un == 840 & a.un == 156 & year == 2017)
# trade.usa.to.china <- subset(trade, i.un == 156 & a.un == 840 & year == 2017)
...
write.xlsx(measures.xlsx, file=paste("0 report production/GTA 23/tables & figures/",output.path,"/Figure ",chapter.number,".3 - Data for Figure ",chapter.number,".3.xlsx", sep=""), row.names=F)
